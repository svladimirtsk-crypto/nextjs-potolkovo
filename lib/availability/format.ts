 ...
 *
 * Страж `scripts/check-availability.mjs` ограничивал запасной файл 21 днём и
 * ронял сборку: список, который правят руками раз в две-три недели, дальше
 * просто неверен. После PT-016 файл запасной, страж только предупреждает, а для
 * БД горизонт шире (60 дней) — обновление больше не требует деплоя, и владелец
 * может занести месяц вперёд одним заходом. Безграничным он не делается по той
 * же причине: дата «через полгода» — это обещание, которое невозможно сдержать.
 */
