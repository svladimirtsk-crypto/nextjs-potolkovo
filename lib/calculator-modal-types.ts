...
export type OpenCalculatorOptions = {
  ...
  forcePreset?: boolean;
  /**
   * Откуда взялся пресет — раздел 3.1 ТЗ. Типизированный union, а не
   * распознавание подстроки в произвольной строке `source`.
   *
   * - `"default"` — заглушка, которую подставляет сам контекст, когда страница
   *   ничего не передала (обычный вход с главной/хедера). О таком
   *   «предзаполнении» клиенту сообщать нечего (N-013, F-10).
   * - `"page"` — пришли со страницы услуги/кейса, пресет частичный.
   * - `"explicit"` — полный пресет конкретного кейса, например
   *   «18 м² / 19 м профиля / 10 м трека».
   *
   * PT-007: значение решает, что делать с сохранённым черновиком. Черновик не
   * применяется и не перезаписывается автоматически ни при каком происхождении
   * пресета — сначала явный выбор пользователя.
   */
