...
const envSchema = z.object({
  ...
  /**
   * PT-020 · Атомарный `claim` заданий outbox (`lease_until` + `SKIP LOCKED`).
   *
   * Правило 8 раздела 2 ТЗ: изменение денежного пути — с флагом отката. `0`
   * возвращает поведение до PT-020: крон берёт задания обычными `SELECT`
   * (`listPendingDeliveries` + `listFailedDeliveries`), аренда не проставляется.
   *
   * Дефолт `1` осознанный: выключенный флаг — это воспроизведённый дефект
   * (два параллельных прогона крона отправляют одну заявку дважды, интеграционный
   * тест это фиксирует). Уникальный индекс `(lead_id, channel)` флагом не
   * выключается — ограничение БД остаётся в силе в любом случае.
   */
  LEAD_OUTBOX_CLAIM_ENABLED: boolFlag(true),
