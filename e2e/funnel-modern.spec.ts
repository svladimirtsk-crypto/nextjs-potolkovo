...
test.describe("Сценарий 2 · современный потолок → подбор света", () => {
  ...
  /**
   * N-051 · Регресс, найденный при написании сценария 2 и исправленный.
   *
   * Если на Шаге 0 трек не выбран, автоочистка T-024 удаляла из корзины любую
   * трековую позицию — включая ту, что клиент только что добавил руками.
   * Счётчик оставался на нуле, «Выбранное (0)», объяснений никаких.
   *
   * Теперь очистка срабатывает только при переходе «трек был → трека нет», а
   * осознанное добавление сопровождается предупреждением с кнопкой убрать.
   */
  test("трековый светильник добавляется, а не исчезает молча", async ({ page }) => {
    await openCalculatorFromHero(page);
    await completeModernStep0(page);
    ...
    // Позиция осталась в корзине.
    await expect(
      modal.locator("button:visible").filter({ hasText: /Выбранное \(1\)/ })
    ).toBeVisible();

    // И клиенту объяснили, что трек не заказан, с явным выбором.
    await modal.locator("button:visible").filter({ hasText: /Выбранное/ }).first().click();
    await expect(modal.getByText(/Вы указали «без трека»/)).toBeVisible();
    await expect(
      modal.getByRole("button", { name: /Убрать трековые позиции/ })
    ).toBeVisible();
  });
});
