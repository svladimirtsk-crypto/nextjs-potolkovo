...
test.describe("Страница услуги", () => {
   ...
   * Тест лежал в `fixme`: квиз открывался с дефолтом прайса 18 м², потому что
   * пресет страницы собирался руками и площадь подменялась на любом входе
   * без `forcePreset`. PT-008 ввёл `EntryContext` (раздел 3.1) — пресет
   * страницы переносится целиком, а дефолт площади действует только для входа
   * `presetOrigin: "default"`. Проверяем и площадь, и тип потолка.
   */
  test("сценарий 3 (полный): CTA услуги предзаполняет теневой профиль и 22 м²", async ({
    page,
  }) => {
    await page.goto(SERVICE);
    await page.getByRole("button", { name: "Рассчитать теневой потолок" }).first().click();
    ...
    await expect(modal.getByLabel("Площадь потолка, м²", { exact: true })).toHaveValue("22");

    // Тип потолка доехал вместе с пресетом: на следующем экране «Теневой»
    // уже выбран, а не предложен кнопкой-раскрытием стандартного сценария.
    await modal.getByRole("button", { name: /Подтвердить площадь/ }).click();
    await expect(modal.getByRole("button", { name: /Теневой потолок/ })).toHaveAttribute(
      "aria-pressed",
      "true"
    );
  });
