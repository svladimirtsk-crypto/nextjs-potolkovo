...
/**
 * COLIBRI открыт по умолчанию, и раньше именно у его светильников поставщик
 * отдавал обложки с ошибкой — поэтому тест смотрит CLARUS_48, систему с полным
 * покрытием фото. PT-017 переверил факт: сейчас локальное фото есть у 100 %
 * товаров с ценой (`npm run check:catalog-images`), так что выбор системы на
 * результат не влияет. Сценарий не меняем — он проверяет отрисовку, а не данные.
 */
