...
`.github/workflows/ci.yml` добавлен владельцем через веб-интерфейс GitHub
(коммит `e5fc955`) — PAT агента не имел scope `workflow`. Содержимое сверено с
подготовленной версией: **идентично**. Jobs: `static` / `build` / `e2e` / `secrets`,
артефакт сборки переиспользуется между `build` и `e2e`.

### Два дефекта, найденных при проверке CI (исправлены в `1ab1268`)

1. **`lefthook` отсутствовал в зависимостях.** `lefthook.yml` лежал в репозитории,
   но пакета не было ни в `package.json`, ни в lock-файле — версия не зафиксирована.
   Добавлен в devDependencies (2.1.12).
2. **Хук gitleaks блокировал бы все коммиты.** `gitleaks` — отдельный бинарь, а не
   npm-пакет; на машине без него pre-commit падал на каждом коммите. Теперь при
   отсутствии бинаря проверка пропускается с сообщением; job `secrets` в CI
   выполняет её в любом случае, поэтому дыры не возникает.

Проверено фактическим прогоном: `npx lefthook validate` → All good, pre-commit
отрабатывает, pre-push реально прогнал 222 теста перед отправкой.
